...
### Дополнение: ChatSettingsManager

В конце миграции перед удалением `Config.cs` необходимо вынести `ChatSettingsManager` (сейчас вложен в тот же файл) в отдельный файл `Infrastructure/ChatSettingsManager.cs` без зависимостей от `Config`.
Шаги:
1. Скопировать текущий класс как есть.
2. Обновить `using` при необходимости.
3. Удалить его из `Config.cs`.
4. Прогнать тесты.
Только после этого удалять сам `Config.cs`.
