...
public class SuspiciousCommandHandler : ICommandHandler
{
    ...
    public async Task HandleAsync(Message message, CancellationToken cancellationToken = default)
    {
        ...
        if (message.Chat.Id != _config.AdminChatId && message.Chat.Id != _config.LogAdminChatId)
            return;
    ...
    private async Task HandleStatsCommand(Message message, CancellationToken cancellationToken)
    {
        ...
        var statusMessage = 
            ...
            $"• Система включена: {(_config.SuspiciousDetectionEnabled ? "✅" : "❌")}\n" +
            $"• Порог мимикрии: *{_config.MimicryThreshold:F1}*\n" +
            $"• Сообщений для одобрения: *{_config.SuspiciousToApprovedMessageCount}*\n\n" +
            $"*Статистика:*\n" +
            $"• Всего подозрительных: *{stats.TotalSuspicious}*\n" +
            $"• С AI детектом: *{stats.WithAiDetect}*\n" +
            $"• Групп: *{stats.GroupsCount}*\n\n" +
            $"*AI анализ:*\n" +
            $"• API настроен: {(_config.OpenRouterApi != null ? "✅" : "❌")}\n" +
            $"• AI чаты включены: *{_config.AiEnabledChats.Count}*\n\n" +
            $"*Команды:*\n" +
