#!/usr/bin/env python3
"""
Скрипт для добавления using ClubDoorman.Services.Statistics в тестовые файлы
"""

import os
import re

def add_statistics_using(file_path):
    """Добавляет using для нового namespace в файл"""
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Проверяем, есть ли уже этот using
    if 'using ClubDoorman.Services.Statistics;' in content:
        print(f"✅ {file_path} - using уже есть")
        return False
    
    # Проверяем, используется ли IStatisticsService или GlobalStatsManager в файле
    if 'IStatisticsService' not in content and 'GlobalStatsManager' not in content:
        print(f"⏭️  {file_path} - IStatisticsService/GlobalStatsManager не используется")
        return False
    ...
    lines.insert(last_using_index + 1, 'using ClubDoorman.Services.Statistics;')
...
def main():
    ...
    files_to_update = [
        ...
        'ClubDoorman.Test/TestInfrastructure/GlobalStatsManagerTestFactory.cs',
        'ClubDoorman.Test/Unit/Services/GlobalStatsManagerTests.cs',
        ...
        'ClubDoorman.Test/Unit/Services/ChannelModerationServiceTests.cs',
    ...
    for file_path in files_to_update:
        if os.path.exists(file_path):
            if add_statistics_using(file_path):
                updated_count += 1
        else:
